...
nrrdSplice(Nrrd *nout, const Nrrd *nin, const Nrrd *nslice,
    ...
    /*
     ...
     what the code is supposed to do (I think): checks that the location in nin where nslice will be inserted is the correct size.
     why it doesn't work (I think): it goes through each axis and compares size, but skips over the axis of insertion. when skipping it causes the iteration to overflow.
     possible fix: subtract one from for loop bounds
     */
